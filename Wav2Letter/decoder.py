...
class GreedyDecoder(Decoder):
        ...
        for i in range(len(sequences)):
            ...
            if return_offsets:
                if i == 0:
                    ...
                    offsets = torch.cat((offsets, string_offsets.unsqueeze(0)), 0)
        if return_offsets:
            return strings, offsets.cuda()
        else:
            return strings
        ...
        for i in range(sequence.size(0)):
            ...
            if char != self.int_to_char[self.blank_index]:
                ...
                if remove_repetitions and i != 0 and char == self.int_to_char[sequence[i - 1].item()]:
                    pass
                elif sequence[i].item() != 1 and len(offsets) < 6:
                    string = string + char
                    offsets.append(sequence[i])
                elif len(offsets) < 6:
                    offsets.append(sequence[i])
        if len(offsets) < 6:
    ...
    def decode(self, ctc_matrix):
        ...
        top = torch.topk(ctc_matrix, k=1, dim=1)

        new_top = top[1][0].detach()
        for i in range(1, top[1].size(0)):
            cur = top[1][i].detach()
            new_top = torch.cat((new_top, cur), 0)
